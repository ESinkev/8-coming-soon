...
for (let letter of name) {
    if (!abc.includes(letter.toLowerCase())) {
        return `Vade panaudota neleistinas simbolis (${letter}).`;
    }

}

// jei gauto vardo raides pavertus mazosiomis yra lygu pradinei reiksmei, tai reiskia, jog vardas ir buvo vien is mazuju raidziu
if (name.toLocaleLowerCase() === name) {
    return 'Vardas negali buti vien tik is mazuju raidziu';
}

// jei pagauname, jog ivesta vien didziosiomis, tai grazinam zinute - isjung CapsLock
